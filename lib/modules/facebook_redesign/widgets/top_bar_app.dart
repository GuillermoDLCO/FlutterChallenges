  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      children: [
        Row(
          children: [
            AvatarProfile(radius: size.width * .042, image: 'assets/facebook_redesign/stories_1.png'),
            Expanded(
              child: Padding(
                padding: const EdgeInsets.symmetric(horizontal: 15.0),
                child: Text(
                  '¿Qué estas pensando, Mao?',
                  style: TextStyle(color: theme.textTheme.bodyText2?.color, fontSize: 14),
                ),
              ),
            ),
            SvgPicture.asset('assets/facebook_redesign/smile.svg', width: size.width * .05)
          ],
        ),
...
class _AppBarCustom extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Row(
      children: [
        SvgPicture.asset('assets/facebook_redesign/facebook_logo.svg', width: MediaQuery.of(context).size.width * .08),
        const Spacer(),
        const _ActionButton(icon: 'assets/facebook_redesign/search.svg'),
        const SizedBox(width: 12.0),
        const _ActionButton(icon: 'assets/facebook_redesign/notifications.svg', hasNotification: true),
        const SizedBox(width: 12.0),
        _ActionButton(icon: 'assets/facebook_redesign/messenger.svg', enableTheme: context.watch<ThemeBloc>().state.appThemeType == AppTheme.Dark),
        const SizedBox(width: 12.0),
        GestureDetector(
            onTap: () => Navigator.of(context).push(
                  CupertinoPageRoute<void>(builder: (BuildContext context) => ProfilePage()),
                ),
            child: AvatarProfile(radius: MediaQuery.of(context).size.width * .042, image: 'assets/facebook_redesign/stories_1.png')),
      ],
    );
  }
}
